...
public class SanityCheck {
	...
	static bool DoSanityCheckPrefab(GameObject gameObject)
	{
		foreach(var sanityCheckComponent in gameObject.GetComponentsInChildren<Component>().Where(c => c is ISanityCheckComponent).Select(c => (ISanityCheckComponent)c)) {
			bool result = sanityCheckComponent.RunSanityCheck(out string message);
	...
	static bool DoSanityCheckWeapon(GameObject weaponPrefab, string name) {
		...
		return DoSanityCheckPrefab(weaponPrefab);
	}
	...
	static bool DoSanityCheckVehicle(GameObject prefab) {
		...
		return DoSanityCheckPrefab(prefab);
	}
	...
	static bool DoSanityCheckMutatorMod(MutatorContentMod mod) {
		foreach(var mutator in mod.mutators) {
			...
			if(!DoSanityCheckPrefab(mutator.mutatorPrefab))
			{
				return false;
			}
		}
	...
	public static bool DoSanityCheck(bool ignoreGraphCacheWarning) {
		...
		var allSanityCheckComponents = GameObject.FindObjectsOfType<Component>().Where(c => c is ISanityCheckComponent).Select(c => (ISanityCheckComponent)c);

		foreach(var sanityCheckComponent in allSanityCheckComponents)
		{
			bool result = sanityCheckComponent.RunSanityCheck(out string message);
