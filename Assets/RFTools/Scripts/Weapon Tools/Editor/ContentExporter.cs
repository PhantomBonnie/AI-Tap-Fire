...
public class ContentExporter : MonoBehaviour {
	...
	static void TestSelected() {
		...
		if(IsContentModObject(Selection.activeGameObject)) {
			...
			if(ok) {
				string parameters = string.Format("-nointro -noworkshopmods -nocontentmods -verbose " +
			...
			if(ok) {
				string parameters = "-nointro -noworkshopmods -nocontentmods -verbose \"-custommap "+path+"\"";
				Paths.LaunchGame(parameters);
			}
			else {
