		...
		if (maps.Count > 0) {
			...
			foreach(var map in maps) {
				if(MetaDataUtils.ReadMetaData(map.FullName, out MapMetaData metaData, MapMetaData.Default)) {
					...
					if(metaData.hasGroundVehicles) {
						this.steamworks.currentItem.tags.Add("Ground Vehicles");
					}
					if (metaData.hasNavalVehicles) {
						this.steamworks.currentItem.tags.Add("Naval Vehicles");
					}
					if (metaData.hasAirVehicles) {
						this.steamworks.currentItem.tags.Add("Air Vehicles");
					}


					if (metaData.mapBiome != MapMetaData.MapBiome.Unknown) {
						this.steamworks.currentItem.tags.Add(metaData.mapBiome.ToString());
					}
					if (metaData.mapDecorator != MapMetaData.MapDecorator.Unknown && metaData.mapDecorator != MapMetaData.MapDecorator.Nothing) {
						this.steamworks.currentItem.tags.Add(metaData.mapDecorator.ToString());
					}
					if (metaData.mapTerrain != MapMetaData.MapTerrain.Unknown) {
						this.steamworks.currentItem.tags.Add(metaData.mapTerrain.ToString());
					}
